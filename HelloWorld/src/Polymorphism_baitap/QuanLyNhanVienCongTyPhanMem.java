...
public class QuanLyNhanVienCongTyPhanMem {
	...
	public static void main(String[] args) {
		...
		for (int i =0; i < soNV; i++) {
			...
			System.out.print("Vi tri (1 - lap trinh vien, 2 - Kiem thu vien): ");
			number = scanner.nextInt();
			while (number != 1 && number != 2) {
				System.out.println("Nhap vi tri khong dung. Nhap lai: ");
				System.out.println("Vi tri (1 - lap trinh vien, 2 - Kiem thu vien): ");
				number = scanner.nextInt();
			}
			switch(number) {
			case 1: 
				nhanVien = new LapTrinhVien();
				nhanVien.setViTri("Lap trinh Vien");
			...
			case 2:
				nhanVien = new QA();
				nhanVien.setViTri("Kiem thu vien");
				nhanVien.nhapThongTin();
				tongLuong += nhanVien.tinhLuong();
				arrNhanVien.add(nhanVien);
				break;
